...
bool SV_CheckBottom (edict_t *ent)
{
	...
	Vector3Add (mins, ent->v.origin, ent->v.mins);
	Vector3Add (maxs, ent->v.origin, ent->v.maxs);
...
bool SV_movestep (edict_t *ent, vec3_t move, bool relink)
{
	...
	Vector3Add (neworg, ent->v.origin, move);
	...
	if ((int) ent->v.flags & (FL_SWIM | FL_FLY))
	{
		...
		{
			Vector3Add (neworg, ent->v.origin, move);
			enemy = ProgToEdict (ent->v.enemy);

			if (i == 0 && enemy != SVProgs->Edicts)
			{
				float dz = ent->v.origin[2] - ProgToEdict (ent->v.enemy)->v.origin[2];
	...
	if (trace.fraction == 1)
	{
		...
		{
			Vector3Add (ent->v.origin, ent->v.origin, move);
...
bool SV_StepDirection (edict_t *ent, float yaw, float dist)
{
	...
	Vector3Copy (oldorigin, ent->v.origin);
	...
	if (SV_movestep (ent, move, false))
	{
		...
		if (delta > 45 && delta < 315)
		{
			...
			Vector3Copy (ent->v.origin, oldorigin);
		}
...
void SV_NewChaseDir (edict_t *actor, edict_t *enemy, float dist)
{
	...
	if (((Q_fastrand () & 3) & 1) ||  abs (deltay) > abs (deltax))
	{
		tdir = d[1];
		d[1] = d[2];
		d[2] = tdir;
	...
	if (Q_fastrand () & 1) 	/*randomly determine direction of search*/
	{
...
void SV_MoveToGoal (void)
{
	...
	goal = ProgToEdict (ent->v.goalentity);
	...
	if (ProgToEdict (ent->v.enemy) != SVProgs->Edicts &&  SV_CloseEnough (ent, goal, dist))
		return;

	// bump around...
	if ((Q_fastrand () & 3) == 1 || !SV_StepDirection (ent, ent->v.ideal_yaw, dist))
	{

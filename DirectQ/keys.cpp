...
// naaaaasssssstyyyyyyy
// this is ugly code and i'll port it to std::ifstream when i get to clean it up
...
{
	std::ofstream histfile (HISTORY_FILE_NAME);
...
void Key_Console (int key)
{
	...
	if ((key == 'V' || key == 'v') && keydown[K_CTRL])
	{
		if (OpenClipboard (NULL))
		{
			...
			if (th)
			{
				...
				if (clipText)
				{
					textCopied = (char *) MainZone->Alloc (GlobalSize (th) + 1);
					strcpy (textCopied, clipText);

					// Substitutes a NULL for every token
					for (;;) if (!strtok (textCopied, "\n\r\b")) break;
	...
	if (key_linepos < MAXCMDLINE - 1)
	{
		...
		if (key_insert)
		{
			...
			if ((i = strlen (key_lines[edit_line]) - 1) >= 254) i = 253;
...
void Key_SetBinding (int keynum, char *binding)
{
	...
	{
		MainZone->Free (keybindings[keynum]);
		keybindings[keynum] = NULL;
	}

	// allocate memory for new binding
	l = strlen (binding);
	newbind = (char *) MainZone->Alloc (l + 1);
...
int Key_GetBinding (char *cmd)
{
	for (int i = 0; i < 256; i++)
	{
		if (!keybindings[i]) continue;
		if (!keybindings[i][0]) continue;
		if (_stricmp (cmd, keybindings[i])) continue;

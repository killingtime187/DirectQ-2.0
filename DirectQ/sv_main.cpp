...
void SV_AllocEdicts (int numedicts)
{
	...
	// edicts are variable sized and we add space for the new edicts
	EdictHunk->Alloc (numedicts * SVProgs->EdictSize);
...
==================
*/
void SV_StartSound (edict_t *entity, int channel, char *sample, int s_volume, float attenuation)
{
	int sound_num;
	int field_mask;
	int	i;
	...
	if (s_volume != DEFAULT_SOUND_PACKET_VOLUME) field_mask |= SND_VOLUME;
	...
	if (field_mask & SND_VOLUME) MSG_WriteByte (&sv.datagram, s_volume);
...
void SV_SendServerinfo (client_t *client)
{
	...
	Q_strncpy (message, SVProgs->GetString (SVProgs->Edicts->v.message), 2047);
...
void SV_ConnectClient (int clientnum)
{
	...
	if (sv.loadgame) Q_MemCpy (spawn_parms, client->spawn_parms, sizeof (float) * NUM_SPAWN_PARMS);
	...
	if (sv.loadgame)
	{
		...
		Q_MemCpy (client->spawn_parms, spawn_parms, sizeof (float) * NUM_SPAWN_PARMS);
	}
	else
	{
		// call the progs to get default spawn parms for the new client
		SVProgs->RunInteraction (NULL, NULL, SVProgs->GlobalStruct->SetNewParms);
...
void SV_WriteEntitiesToClient (client_t *client, sizebuf_t *msg)
{
	...
	// DP_SV_CLIENTCAMERA
	edict_t *clent = GetEdictForNumber (client->clientcamera);
	vec3_t org;

	// find the client's PVS
	Vector3Add (org, clent->v.origin, clent->v.view_ofs);
	...
	for (int e = 1; e < SVProgs->NumEdicts; e++, ent = NextEdict (ent))
	{
		// don't write free edicts
		if (ent->free) continue;

		// ignore if not touching a PV leaf (client is always sent)
		if (ent != clent)
		{
			// ignore ents without visible models (client is always sent as it may be valid for it to have no model (e.g. in an intermission))
			if (!ent->v.modelindex || !SVProgs->GetString (ent->v.model)[0]) continue;

			// reversion to the old way to combat excessive CPU load
			// fixme - implement the new RMQ way
			if (!sv_novis.value && leaf->contents != CONTENTS_SOLID)
			{
				int i = 0;
		...
		// restrict movetype step behaviour on client to ents that are actually step-lerped
		if (ent->v.movetype == MOVETYPE_STEP && ((int) ent->v.flags & (FL_ONGROUND | FL_FLY | FL_SWIM))) bits |= U_NOLERP;
...
void SV_ClearMuzzleFlashes (void)
{
	...
	ent = NextEdict (SVProgs->Edicts);

	for (e = 1; e < SVProgs->NumEdicts; e++, ent = NextEdict (ent))
	{
...
void SV_WriteClientdataToMessage (edict_t *ent, sizebuf_t *msg)
{
	...
	{
		other = ProgToEdict (ent->v.dmg_inflictor);
...
bool SV_SendClientDatagram (client_t *client)
{
	...
	byte *buf = (byte *) TempHunk->FastAlloc (MAX_DATAGRAM);
...
void SV_SendNop (client_t *client)
{
	...
	client->last_message = CHostTimer::realtime;
}
...
void SV_SendClientMessages (void)
{
	...
	for (i = 0, host_client = svs.clients; i < svs.maxclients; i++, host_client++)
	{
		...
		else
		{
			...
			{
				if (CHostTimer::realtime - host_client->last_message > 5)
		...
		if (host_client->message.cursize || host_client->dropasap)
		{
			...
			else
			{
				...
				host_client->last_message = CHostTimer::realtime;
...
void SV_CreateBaseline (void)
{
	...
	for (entnum = 0; entnum < SVProgs->NumEdicts; entnum++)
	{
		...
		Vector3Copy (svent->baseline.origin, svent->v.origin);
		Vector3Copy (svent->baseline.angles, svent->v.angles);
...
void SV_SaveSpawnparms (void)
{
	...
	for (i = 0, host_client = svs.clients; i < svs.maxclients; i++, host_client++)
	{
		...
		// call the progs to get default spawn parms for the new client
		SVProgs->RunInteraction (host_client->edict, NULL, SVProgs->GlobalStruct->SetChangeParms);
...
void SV_SpawnServer (char *server)
{
	...
	if (hostname.string[0] == 0) hostname.Set ("UNNAMED");
	...
	if (coop.value) deathmatch.Set (0.0f);
	...
	spawnserver = true;
	Q_snprintf (sv.modelname, 64, "maps/%s.bsp", server);
	...
	if (EdictHunk) delete EdictHunk;

	// add some extra space for overhead
	int edictsizemb = (MAX_EDICTS + 1) * SVProgs->EdictSize;

	edictsizemb = (edictsizemb + 1023) >> 10;
	edictsizemb = (edictsizemb + 1023) >> 10;

	EdictHunk = new CQuakeHunk (edictsizemb);

	// get a base pointer for all edicts
	SVProgs->Edicts = (edict_t *) EdictHunk->Alloc (1);
	...
	Con_DPrintf ("SV_SpawnServer took %f seconds\n", Sys_DoubleTime () - beginloadtime);
}

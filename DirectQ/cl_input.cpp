...
void IN_MLookUp (void)
{
	...
	if (!(freelook.integer || (in_mlook.state & 1)) && lookspring.value)
		V_StartPitchDrift ();
}
...
void CL_SendLagMove (void)
{
	...
	while ((lag_tail < lag_head) && (lag_sendtime[lag_tail & 31] <= CHostTimer::realtime))
	{
		lag_tail++;
...
#if 1
		// don't check for lost connection here (by definition this is an unreliable message so it should be allowed fail)
...
void CL_SendMove (usercmd_t *cmd)
{
	...
	if (pq_lag.value < 0) pq_lag.Set (0.0f);
	if (pq_lag.value > 400) pq_lag.Set (400.0f);

	buf = &lag_buff[lag_head & 31];
	SZ_Init (buf, lag_data[lag_head & 31], sizeof (lag_data[lag_head & 31]));
	lag_sendtime[(lag_head++) & 31] = CHostTimer::realtime + (pq_lag.value / 1000.0f);
	...
	else
	{
		...
		MSG_WriteAngle (buf, cl.viewangles[2], cl.Protocol, cl.PrototcolFlags, 0);
	}

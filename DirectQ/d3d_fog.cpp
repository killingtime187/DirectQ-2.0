...
// fog globals because state may modify them
float RealFogDensity = 0;
float *RealFogColor = NULL;
float SkyFogDensity = 0;;

float d3d_FogOldColor[4] = {0.3f, 0.3f, 0.3f, 0.0f};
float d3d_FogOldDensity = 0.0f;
float d3d_FogColor[4] = {0.3f, 0.3f, 0.3f, 0.0f};
float d3d_FogDensity = 0.0f;
float d3d_FogTime = 0.0f;
char lastworldmodel[64] = {0};

// underwater fog/etc
extern float r_underwaterfogcolours[];
...
void Fog_Update (float density, float r, float g, float b, float time)
{
	...
	if (d3d_FogDensity > 0)
	{
		gl_fogenable.Set (1.0f);
		gl_fogdensity.Set (d3d_FogDensity);
	}
	else gl_fogenable.Set (0.0f);

	gl_fogred.Set (d3d_FogColor[0]);
	gl_foggreen.Set (d3d_FogColor[1]);
	gl_fogblue.Set (d3d_FogColor[2]);

	// now unlock the cvars and do another update to sync things up right
	fog_LockCvars = false;
	gl_fogenable.Set (gl_fogenable.value);
}


void Fog_ParseWorldspawn (void)
{
	char key[1025], value[4096];
	char *data;
	...
	for (;;)
	{
		...
		if (!strcmp ("fog", key))
		{
			if (sscanf (value, "%f %f %f %f", &d3d_FogDensity, &d3d_FogColor[0], &d3d_FogColor[1], &d3d_FogColor[2]) == 4)
			{
...
void D3DFog_BeginFrame (void)
{
	// initially no fog
	RealFogDensity = 0;
	RealFogColor = d3d_FogColor;

	if ((d3d_RenderDef.viewleaf->contents == CONTENTS_SOLID) || (d3d_RenderDef.viewleaf->contents == CONTENTS_SKY))
	{
		// disable fog too if noclipping
		RealFogDensity = 0;
	}
	else if (r_underwaterfogcolours[3] > 0)
	{
		// Con_Printf ("enabled underwater fog\n");
		// approximate; looks OK
		RealFogColor = r_underwaterfogcolours;
		SkyFogDensity = 0.975f;

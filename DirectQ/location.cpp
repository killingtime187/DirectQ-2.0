...
void LOC_LoadLocations (void)
{
	...
	Q_MemCpy (locname, "locs", 4);
	COM_DefaultExtension (locname, ".loc");

	char *locdataload = (char *) CQuakeFile::LoadFile (locname);
	...
	if (numlocations > 0)
	{
		l = (location_t *) MainHunk->Alloc (numlocations * sizeof (location_t));
		Q_MemCpy (l, locations, numlocations * sizeof (location_t));
		locations = l;
	}
	else locations = NULL;

	MainZone->Free (locdataload);

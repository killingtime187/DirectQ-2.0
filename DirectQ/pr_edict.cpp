...
void ED_Free (edict_t *ed)
{
	...
	ed->v.skin = 0;
	ed->v.frame = 0;
	Vector3Copy (ed->v.origin, vec3_origin);
	Vector3Copy (ed->v.angles, vec3_origin);
	ed->v.nextthink = -1;
	ed->v.solid = 0;
	ed->num_leafs = 0;
...
{
	for (int i = 0; i < SVProgs->QC->numglobaldefs; i++)
	{
		ddef_t *def = &SVProgs->GlobalDefs[i];
...
ddef_t *ED_FieldAtOfs (int ofs)
{
	for (int i = 0; i < SVProgs->QC->numfielddefs; i++)
	{
		ddef_t *def = &SVProgs->FieldDefs[i];
...
{
	for (int i = 0; i < SVProgs->QC->numfunctions; i++)
	{
		dfunction_t *func = &SVProgs->Functions[i];
...
char *PR_ValueString (etype_t type, eval_t *val)
{
	...
	{
	case ev_string:
		Q_snprintf (line, 256, "%s", SVProgs->GetString (val->string));
		break;
	case ev_entity:
		Q_snprintf (line, 256, "entity %i", GetNumberForEdict (ProgToEdict (val->edict)));
		break;
	case ev_function:
		f = SVProgs->Functions + val->function;
		Q_snprintf (line, 256, "%s()", SVProgs->GetString (f->s_name));
		break;
	case ev_field:
		def = ED_FieldAtOfs (val->_int);
		Q_snprintf (line, 256, ".%s", SVProgs->GetString (def->s_name));
		break;
	case ev_void:
		Q_snprintf (line, 256, "void");
		break;
	case ev_float:
		Q_snprintf (line, 256, "%5.1f", val->_float);
		break;
	case ev_vector:
		Q_snprintf (line, 256, "'%5.1f %5.1f %5.1f'", val->vector[0], val->vector[1], val->vector[2]);
		break;
	case ev_pointer:
		Q_snprintf (line, 256, "pointer");
		break;
	default:
		Q_snprintf (line, 256, "bad type %i", itype);
		break;
	}
...
/*
============
PR_WriteUglyValueString

Writes a string describing *data in a type specific manner to an output stream
Easier to parse than PR_ValueString
=============
*/
void PR_WriteUglyValueString (std::ofstream &f, etype_t type, eval_t *val)
{
	int itype = (int) type;
	...
	{
	case ev_string:
		f << SVProgs->GetString (val->string);
		break;

	case ev_entity:
		f << GetNumberForEdict (ProgToEdict (val->edict));
		break;

	case ev_function:
		f << SVProgs->GetString ((SVProgs->Functions + val->function)->s_name);
		break;

	case ev_field:
		f << SVProgs->GetString (ED_FieldAtOfs (val->_int)->s_name);
		break;
	...
	case ev_vector:
		f << val->vector[0] << " " << val->vector[1] << " " << val->vector[2];
		break;
...
char *PR_GlobalString (int ofs)
{
	...
	if (!def)
		Q_snprintf (line, 128, "%i(???)", ofs);
	else
	{
		s = PR_ValueString ((etype_t) def->type, (eval_t *) val);
		Q_snprintf (line, 128, "%i(%s)%s", ofs, SVProgs->GetString (def->s_name), s);
	}
...
char *PR_GlobalStringNoContents (int ofs)
{
	...
	if (!def)
		Q_snprintf (line, 128, "%i(???)", ofs);
	else Q_snprintf (line, 128, "%i(%s)", ofs, SVProgs->GetString (def->s_name));
...
void ED_Write (std::ofstream &f, edict_t *ed)
{
	...
	if (!ed->free)
	{
		for (int i = 1; i < SVProgs->QC->numfielddefs; i++)
		{
			ddef_t *d = &SVProgs->FieldDefs[i];
			char *name = SVProgs->GetString (d->s_name);
			...
			for (int j = 0; j < type_size[type]; j++)
			{
				...
				{
					f << "\"" << name << "\" ";
					PR_WriteUglyValueString (f, (etype_t) d->type, (eval_t *) v);
					f << "\n";
				}
			}
		}
...
void ED_WriteGlobals (std::ofstream &f)
{
	...
	for (int i = 0; i < SVProgs->QC->numglobaldefs; i++)
	{
		...
		f << "\"" << SVProgs->GetString (def->s_name) << "\" ";
		PR_WriteUglyValueString (f, (etype_t) type, (eval_t *) &SVProgs->Globals[def->ofs]);
		f << "\n";
	}
...
=============
*/
char *ED_ParseGlobals (char *data)
{
	char	keyname[1025];
	ddef_t	*key;
	...
	for (;;)
	{
		...
		// parse value
		if (!(data = COM_Parse (data))) Sys_Error ("ED_ParseEntity: EOF without closing brace");
...
bool ED_ParseEpair (void *base, ddef_t *key, char *s)
{
	...
	case ev_entity:
		*(int *) d = EdictToProg (GetEdictForNumber (atoi (s)));
		break;
...
char *ED_ParseEdict (char *data, edict_t *ent)
{
	...
	for (;;)
	{
		...
		if (anglehack)
		{
			...
			Q_snprintf (com_token, 1024, "0 %s 0", temp);
		}
...
void ED_LoadFromFile (char *data)
{
	...
	for (;;)
	{
		...
		dfunction_t *func = ED_FindFunction (classname);
		...
		SVProgs->RunInteraction (ent, NULL, func - SVProgs->Functions);
		ed_number++;
	}
...
{
	int b = (byte *) e - (byte *) SVProgs->Edicts;

	b /= SVProgs->EdictSize;

	if (b < 0 || b >= SVProgs->MaxEdicts)
		Host_Error ("GetEdictForNumber: bad number %i (max: %i)", b, SVProgs->MaxEdicts);

	return b;
}


int EdictToProg (edict_t *e)
{
	return ((byte *) e - (byte *) SVProgs->Edicts);
}


edict_t *ProgToEdict (int prog)
{
	return (edict_t *) ((byte *) SVProgs->Edicts + prog);
}


edict_t *NextEdict (edict_t *e)
{
	return (edict_t *) ((byte *) e + SVProgs->EdictSize);
}

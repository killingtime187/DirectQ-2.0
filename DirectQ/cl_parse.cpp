...
entity_t *CL_EntityNum (int num)
{
	if (num >= cl.num_entities)
	{
		...
		{
			if (!cls.entities[cl.num_entities])
			{
				// alloc a new entity and set it's number
				cls.entities[cl.num_entities] = (entity_t *) MainHunk->Alloc (sizeof (entity_t));
				cls.entities[cl.num_entities]->entnum = cl.num_entities;
				cls.entities[cl.num_entities]->alphaval = 0;
			}

			// force cl.numentities up until it exceeds the number we're looking for
			cls.entities[cl.num_entities]->lerpflags |= LERP_RESETALL;
			cl.num_entities++;
		}
	}

	return cls.entities[num];
}
...
void CL_ParseStartSoundPacket (void)
{
	...
	if (field_mask & SND_VOLUME)
		s_volume = MSG_ReadByte ();
	else s_volume = DEFAULT_SOUND_PACKET_VOLUME;
	...
	S_StartSound (ent, channel, cl.sound_precache[sound_num], pos, s_volume / 255.0, attenuation);
}
...
void CL_KeepaliveMessage (bool showmsg = true)
{
	...
	// here we just leak the memory for now and let the per-frame cleanup handle it in case anything else
	// needs to use temphunk as a result of this message
	byte *olddata = (byte *) TempHunk->FastAlloc (net_message.cursize);
...
void CL_DoWebDownload (char *filename)
{
	...
	if (DLResult == DL_ERR_NO_ERROR)
	{
		...
		std::ifstream f (va ("%s/%s", com_gamedir, filename), std::ios::in | std::ios::binary);

		if (f.is_open ())
		{
			dheader_t h;
			f.read ((char *) &h, sizeof (dheader_t));
			f.close ();
...
void CL_ParseServerInfo (void)
{
	...
	// cleared here so that MainHunk is valid to use for allocs in this function
	...
	model_precache = (char **) MainHunk->Alloc (MAX_MODELS * sizeof (char *));
	sound_precache = (char **) MainHunk->Alloc (MAX_SOUNDS * sizeof (char *));
	...
	for (i = 0; i < MAX_MODELS; i++) model_precache[i] = NULL;
	for (i = 0; i < MAX_SOUNDS; i++) sound_precache[i] = NULL;
	...
	cl.scores = (scoreboard_t *) MainHunk->Alloc (MAX_SCOREBOARD * sizeof (scoreboard_t));
	...
	str = MSG_ReadString ();
	cl.levelname = (char *) MainHunk->Alloc (strlen (str) + 1);
	...
	for (nummodels = 1;; nummodels++)
	{
		...
		model_precache[nummodels] = (char *) MainHunk->Alloc (strlen (str) + 1);
	...
	for (numsounds = 1;; numsounds++)
	{
		...
		sound_precache[numsounds] = (char *) MainHunk->Alloc (strlen (str) + 1);
	...
	cls.entities[0]->model = cl.worldmodel = cl.model_precache[1];

	char mapname[MAX_PATH];

	cl.worldmodel->name[63] = 0;
	Q_strncpy (mapname, cl.worldmodel->name, MAX_PATH - 1);

	for (i = strlen (cl.worldmodel->name); i; i--)
	{
...
void CL_ParseUpdate (int bits)
{
	...
	// this is used for both getting an existing entity and creating a new one.  eeewww.
	entity_t *ent = CL_EntityNum (i);

	// entity was not present on the previous frame
	bool forcelink = (ent->curr.msgtime != cl.mtime[1]);

	if (ent->curr.msgtime + 0.2 < cl.mtime[0])
	{
		// more than 0.2 seconds since the last message (most entities think every 0.1 sec)
		ent->lerpflags |= LERP_RESETALL;
	}

	ent->prev.msgtime = ent->curr.msgtime;
	ent->curr.msgtime = cl.mtime[0];
	...
	if (bits & U_MODEL)
	{
		...
		{
			Con_DPrintf ("CL_ParseModel : bad modnum\n");
	...
	// shuffle the values for interpolation
	// origin and angles are always updated - even if a message is not sent, they just reset to baseline - so the
	// shuffle is always needed even if a message is not sent
	Vector3Copy (ent->prev.msg_origin, ent->curr.msg_origin);
	Vector3Copy (ent->prev.msg_angles, ent->curr.msg_angles);

	if (bits & U_ORIGIN1)
		ent->curr.msg_origin[0] = MSG_ReadCoord (cl.Protocol, cl.PrototcolFlags);
	else ent->curr.msg_origin[0] = ent->baseline.origin[0];

	if (bits & U_ANGLES1)
		ent->curr.msg_angles[0] = MSG_ReadAngle (cl.Protocol, cl.PrototcolFlags);
	else ent->curr.msg_angles[0] = ent->baseline.angles[0];

	if (bits & U_ORIGIN2)
		ent->curr.msg_origin[1] = MSG_ReadCoord (cl.Protocol, cl.PrototcolFlags);
	else ent->curr.msg_origin[1] = ent->baseline.origin[1];

	if (bits & U_ANGLES2)
		ent->curr.msg_angles[1] = MSG_ReadAngle (cl.Protocol, cl.PrototcolFlags);
	else ent->curr.msg_angles[1] = ent->baseline.angles[1];

	if (bits & U_ORIGIN3)
		ent->curr.msg_origin[2] = MSG_ReadCoord (cl.Protocol, cl.PrototcolFlags);
	else ent->curr.msg_origin[2] = ent->baseline.origin[2];

	if (bits & U_ANGLES3)
		ent->curr.msg_angles[2] = MSG_ReadAngle (cl.Protocol, cl.PrototcolFlags);
	else ent->curr.msg_angles[2] = ent->baseline.angles[2];
	...
	else if (ent != cls.entities[cl.viewentity])
		ent->alphaval = 0;
	...
	if (model != ent->model)
	{
		...
		if (model)
		{
			if (model->synctype == ST_RAND)
				ent->syncbase = (float) (Q_fastrand () & 0x7fff) / 0x7fff;
	...
	// don't movetype_step non-alias models
	if ((bits & U_NOLERP) && (ent->model) && ((ent->model->type == mod_alias) || (ent->model->type == mod_iqm)))
		ent->lerpflags |= LERP_MOVESTEP;
	else ent->lerpflags &= ~LERP_MOVESTEP;

	if (ent->model) ent->model->numents = 0;
...
void CL_ParseClientdata (void)
{
	...
	Vector3Copy (cl.mvelocity[1], cl.mvelocity[0]);
	Vector3Copy (cl.mpunchangle[1], cl.mpunchangle[0]);

	for (i = 0; i < 3; i++)
	{
		if (bits & (SU_PUNCH1 << i))
			cl.mpunchangle[0][i] = MSG_ReadChar ();
		else cl.mpunchangle[0][i] = 0;
	...
	if (cl.stats[STAT_HEALTH] > 0 && clnewstats[STAT_HEALTH] < 1)
	{
		...
		cl.death_location[0] = cls.entities[cl.viewentity]->origin[0];
		cl.death_location[1] = cls.entities[cl.viewentity]->origin[1];
		cl.death_location[2] = cls.entities[cl.viewentity]->origin[2];
...
void CL_ParseStatic (int version)
{
	...
	{
		Con_Printf ("CL_ParseStatic : invalid modelindex: %i\n", ent->baseline.modelindex);
		return;
	}
	...
	if (ent->model)
	{
		// statics should also have a syncbase and it's kept consistent with non-statics
		...
		if (ent->model->type == mod_alias || ent->model->type == mod_iqm) D3DLight_PrepStaticEntityLighting (ent);
	}
}
...
char cl_notifystring[1024];
cvar_t cl_shownotify ("cl_shownotify", "1", CVAR_ARCHIVE);
...
{
	if (cl_notifystring[0] && (!vid.ActiveApp || vid.Minimized) && cl_shownotify.value)
	{
		Sys_NotifyIcon (NIM_MODIFY, "DirectQ", cl_notifystring);
		cl_notifystring[0] = 0;
	}
}
...
void CL_ParseServerMessage (void)
{
	...
	for (;;)
	{
		...
		case svc_time:
			// entities will be positioned at an interpolated point between these two times
		...
		case svc_print:
			if (!cls.download.web)
			{
				...
				if (strlen (cl_notifystring) + strlen (str) < 1023)
					strcat (cl_notifystring, str);

				// proquake messaging only exists with protocol 15 in MP games
				if (cl.Protocol == PROTOCOL_VERSION_NQ && cl.maxclients > 1)
				{
		...
		case svc_lightstyle:
			...
			Q_strncpy ((char *) cls.lightstyles[i].map,  MSG_ReadString (), 64);
			cls.lightstyles[i].length = strlen ((char *) cls.lightstyles[i].map);
			break;
		...
		case svc_spawnbaseline:
			...
			// clear the interpolation data to get a valid baseline
			(CL_EntityNum (i))->lerpflags |= LERP_RESETALL;
...
void Q_Version (char *s)
{
	...
	if (CHostTimer::realtime - q_version_reply_time < 20)
		return;
	...
	while (n < l)
	{
		if (!strncmp (t, ": q_version", 9))
		{
			...
			Cbuf_Execute ();
			q_version_reply_time = CHostTimer::realtime;

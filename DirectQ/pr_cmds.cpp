...
void QC_DebugOutput (char *debugtext, ...)
{
	...
	{
		std::ofstream f ("qcdebuglog.txt", std::ios::out | std::ios::app);
...
#define	RETURN_EDICT(e) (((int *) SVProgs->Globals)[OFS_RETURN] = EdictToProg(e))
...
char *PR_GetTempString (void)
{
	...
	if (!pr_temp_strings)
	{
		...
		pr_temp_strings = (char **) MainZone->Alloc (sizeof (char *) * PR_NUM_TEMP_STRINGS);

		for (int i = 0; i < PR_NUM_TEMP_STRINGS; i++) pr_temp_strings[i] = (char *) MainZone->Alloc (PR_MAX_TEMP_STRING);
	}

	// go to a new temp string, rotate the buffer if needed, and ensure that it's initially null termed
...
{
	// use our own string pool so that this memory gets properly walled off
	static int pf_varstring_level = 0;
	static char pf_varstring_pool[8][PR_MAX_TEMP_STRING];

	// this should never be needed...
	char *out = pf_varstring_pool[(++pf_varstring_level) & 7];
	...
	for (int i = first, len = 0; i < SVProgs->Argc; i++)
	{
		...
		// prevent buffer overflow in this function (note: += is intentional)
...
void PF_error (void)
{
	...
	ed = ProgToEdict (SVProgs->GlobalStruct->self);
	ED_Print (ed);
...
void PF_makevectors (void)
{
	...
	av.AngleVectors (G_VECTOR (OFS_PARM0));

	// the globals table has the right and up vectors out of order - grrrrr
	Vector3Copy (SVProgs->GlobalStruct->v_fw, av.fw);
	Vector3Copy (SVProgs->GlobalStruct->v_rt, av.rt);
	Vector3Copy (SVProgs->GlobalStruct->v_up, av.up);
}
...
void SetMinMaxSize (edict_t *e, float *mins, float *maxs, bool rotate)
{
	vec3_t	rmin, rmax;
	...
	if (!rotate)
	{
		Vector3Copy (rmin, mins);
		Vector3Copy (rmax, maxs);
	}
	else SV_RotateBBoxToBBox (e, mins, maxs, rmin, rmax);

	// set derived values
	Vector3Copy (e->v.mins, rmin);
	Vector3Copy (e->v.maxs, rmax);
	Vector3Subtract (e->v.size, maxs, mins);
...
void PF_setsize (void)
{
	...
	SetMinMaxSize (e, mins, maxs, false);
}
...
void PF_normalize (void)
{
	...
	Vector3Copy (G_VECTOR (OFS_RETURN), newvalue);
}
...
void PF_vectoangles (void)
{
	...
	{
		yaw = (atan2 (value1[1], value1[0]) * 180) / D3DX_PI;
		...
		pitch = (atan2 (value1[2], forward) * 180) / D3DX_PI;
...
void PF_random (void)
{
	...
	num = (Q_fastrand () & 0x7fff) / ((float) 0x7fff);
...
void PF_sound (void)
{
	...
	s_volume = G_FLOAT (OFS_PARM3) * 255;
	attenuation = G_FLOAT (OFS_PARM4);

	if (s_volume < 0 || s_volume > 255)
		Sys_Error ("SV_StartSound: volume = %i", s_volume);
	...
	SV_StartSound (entity, channel, sample, s_volume, attenuation);
}
...
void PF_traceline (void)
{
	...
	Vector3Copy (SVProgs->GlobalStruct->trace_endpos, trace.endpos);
	Vector3Copy (SVProgs->GlobalStruct->trace_plane_normal, trace.plane.normal);

	SVProgs->GlobalStruct->trace_plane_dist =  trace.plane.dist;

	if (trace.ent)
		SVProgs->GlobalStruct->trace_ent = EdictToProg (trace.ent);
	else SVProgs->GlobalStruct->trace_ent = EdictToProg (SVProgs->Edicts);
}
...
int PF_newcheckclient (int check)
{
	...
	Vector3Add (org, ent->v.origin, ent->v.view_ofs);
...
void PF_checkclient (void)
{
	...
	edict_t *ent = GetEdictForNumber (sv.lastcheck);
	extern cvar_t sv_novis;

	if (ent->free || ent->v.health <= 0 || ent->v.movetype == MOVETYPE_NOCLIP)
	{
		// can never see it
	...
	edict_t *self = ProgToEdict (SVProgs->GlobalStruct->self);
	vec3_t view;

	Vector3Add (view, self->v.origin, self->v.view_ofs);

	mleaf_t *leaf = Mod_PointInLeaf (view, sv.worldmodel);
	int l = (leaf - sv.worldmodel->brushhdr->leafs) - 1;
...
void PF_stuffcmd (void)
{
	...
	Host_ClientCommands (&svs.clients[entnum - 1], "%s", str);
}
...
void PF_cvar (void)
{
	...
	G_FLOAT (OFS_RETURN) = cvar_t::VariableValue (str);
}
...
void PF_cvar_set (void)
{
	...
	if ((cv = cvar_t::FindVar (var)) != NULL)
	{
		cv->Set (val);
		QC_DebugOutput ("Setting cvar \"%s\" to \"%s\"", var, val);
	}
	else Con_Printf ("\"%s\" is not a cvar\n", var);
}
...
{
	edict_t *chain = SVProgs->Edicts;
	float *org = G_VECTOR (OFS_PARM0);
	float rad = G_FLOAT (OFS_PARM1);
	edict_t *ent = NextEdict (SVProgs->Edicts);

	// substitute Vector3Length with Vector3Dot
	...
	for (int i = 1; i < SVProgs->NumEdicts; i++, ent = NextEdict (ent))
	{
		...
		float eorg[3] =
		{
			org[0] - (ent->v.origin[0] + (ent->v.mins[0] + ent->v.maxs[0]) * 0.5),
			org[1] - (ent->v.origin[1] + (ent->v.mins[1] + ent->v.maxs[1]) * 0.5),
			org[2] - (ent->v.origin[2] + (ent->v.mins[2] + ent->v.maxs[2]) * 0.5)
		};

		// substitute Vector3Length with Vector3Dot
		if (Vector3Dot (eorg, eorg) < radius2)
		{
			ent->v.chain = EdictToProg (chain);
			chain = ent;
		}
	}
...
void PF_ftos (void)
{
	...
	if (v == (int) v)
		Q_snprintf (pr_string_temp, 128, "%d", (int) v);
	else
		Q_snprintf (pr_string_temp, 128, "%f", v);
...
void PF_vtos (void)
{
	...
	Q_snprintf (pr_string_temp, 128, "'%5.1f %5.1f %5.1f'", G_VECTOR (OFS_PARM0)[0], G_VECTOR (OFS_PARM0)[1], G_VECTOR (OFS_PARM0)[2]);
	G_INT (OFS_RETURN) = SVProgs->SetString (pr_string_temp);
}
...
void PF_etos (void)
{
	...
	Q_snprintf (pr_string_temp, 128, "entity %i", G_EDICTNUM (OFS_PARM0));
	G_INT (OFS_RETURN) = SVProgs->SetString (pr_string_temp);
}
...
void PF_Find (void)
{
	...
	if (!s) SVProgs->RunError ("PF_Find: bad search string");

	for (e++; e < SVProgs->NumEdicts; e++)
	{
		if ((ed = GetEdictForNumber (e))->free) continue;
		if (!(t = E_STRING (ed, f))) continue;
...
void PF_walkmove (void)
{
	...
	ent = ProgToEdict (SVProgs->GlobalStruct->self);
...
void PF_aim (void)
{
	...
	Vector3Copy (start, ent->v.origin);
	start[2] += 20;

	// try sending a trace straight
	Vector3Copy (dir, SVProgs->GlobalStruct->v_fw);
	Vector3Mad (end, dir, 2048, start);
	...
	{
		Vector3Copy (G_VECTOR (OFS_RETURN), SVProgs->GlobalStruct->v_fw);
		return;
	}

	// try all possible entities
	Vector3Copy (bestdir, dir);
	...
	for (i = 1; i < SVProgs->NumEdicts; i++, check = NextEdict (check))
	{
		...
		Vector3Subtract (dir, end, start);
		Vector3Normalize (dir);

		if ((dist = Vector3Dot (dir, SVProgs->GlobalStruct->v_fw)) < bestdist) continue;	// to far to turn
	...
	if (bestent)
	{
		Vector3Subtract (dir, bestent->v.origin, ent->v.origin);

		dist = Vector3Dot (dir, SVProgs->GlobalStruct->v_fw);
		Vector3Scale (end, SVProgs->GlobalStruct->v_fw, dist);

		end[2] = dir[2];

		Vector3Normalize (end);
		Vector3Copy (G_VECTOR (OFS_RETURN), end);
	}
	else Vector3Copy (G_VECTOR (OFS_RETURN), bestdir);
}
...
sizebuf_t *WriteDest (void)
{
	...
	case MSG_ONE:
		ent = ProgToEdict (SVProgs->GlobalStruct->msg_entity);
...
void PF_stov (void)
{
	...
	if (!(v = G_STRING (OFS_PARM0)))
	{
		Vector3Copy (G_VECTOR (OFS_RETURN), vec3_origin);
		return;
	}
	...
	Vector3Copy (G_VECTOR (OFS_RETURN), d);
}
...
void PF_strzone (void)
{
	...
	p = (char *) MainZone->Alloc (strlen (m) + 1);
...
{
	// the compiler freaks if I pass this directly into MainZone->Free...
...
void PF_strcat (void)
{
	...
	{
		Q_strncpy (pr_string_temp, s1, PR_MAX_TEMP_STRING);
...
int pr_ebfs_numbuiltins = ARRAYLENGTH (pr_ebfs_builtins);
// 2001-09-14 Enhanced BuiltIn Function System (EBFS) by Maddes  end

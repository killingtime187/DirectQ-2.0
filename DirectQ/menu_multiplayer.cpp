...
void Menu_SetupApplyFunc (void)
{
	...
	hostname.Set (dummy_hostname.string);
...
void Menu_SetupCustomEnter (void)
{
	...
	// copy cvars out
	dummy_name.Set (cl_name.string);
	dummy_hostname.Set (hostname.string);
...
{
	// copy out the port
	dummy_port.Set (va ("%i", DEFAULTnet_hostport));
	dummy_remoteip.Set ("0");
}
...
void Menu_GameConfigBeginGame (void)
{
	...
	timelimit.Set (dummy_timelimit.value);
	fraglimit.Set (dummy_fraglimit.value);
	skill.Set ((float) dummy_skill);
	teamplay.Set ((float) dummy_teamplay);
...
int Menu_SearchCustomDraw (int y)
{
	...
	if (!Menu_SearchComplete)
	{
		...
		Menu_SearchCompleteTime = CHostTimer::realtime;
	}
	...
	if ((CHostTimer::realtime - Menu_SearchCompleteTime) < 3.0) return y;
...
void Menu_SListOnHover (int initialy, int y, int itemnum)
{
	...
	// rest of info
	Draw_Mapshot ("maps/", hc->map, (vid.currsize.width - 320) / 2 + 208, initialy + 8);
	Menu_PrintWhite (228, initialy + 145, "Server Info");
	Menu_Print (218, initialy + 160, DIVIDER_LINE);

	char hostipconfig[1025];
	char *hostport;
...
void NET_MenuReturn (void)
{
	if (Menu_NetReturn)
	{
		Net_ErrorReturnTime = CHostTimer::realtime + 5;
		Menu_StackPop ();
	}
...
{
	if (Net_ErrorReturnTime < CHostTimer::realtime) return y;
...
{
	y += 5;
	Draw_Mapshot ("maps/", spinbox_bsps[map_number], (vid.currsize.width - 320) / 2 + 96, y);
	y += 130;

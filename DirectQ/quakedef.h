...
// force conditional expression is constant to level 1 to avoid need for "if (NULL == variable)" horror
#pragma warning (1: 4127)

// defined as early as possible as it's needed in multiple headers
#define	MAX_DLIGHTS		128

#define ARRAYLENGTH(sa) (sizeof (sa) / sizeof ((sa)[0]))

// get rid of a lot of cruddy code
void Mod_ClearBoundingBox (float *mins, float *maxs);
void Mod_ClearBoundingBox (struct cullinfo_t *cull);
...
void Mod_AccumulateBox (struct cullinfo_t *bbcull, struct cullinfo_t *pcull);
...
// override d3dmatrix with our own version for a more flexible union
struct D3DMATRIX
{
	union
	{
		...
		// need to keep m as the d3d stuff depends on it... (ugh) - but m4x4 is there as an option
		float m[4][4];
		float m4x4[4][4];
		float m16[16];
	};
...
// this prevents D3DMATRIX from being defined in the d3d headers
...
public:
	QCOLOR (void);
	QCOLOR (unsigned char _r, unsigned char _g, unsigned char _b);
	QCOLOR (unsigned char _r, unsigned char _g, unsigned char _b, unsigned char _a);
	QCOLOR (unsigned char *_rgba);
	QCOLOR (unsigned int _color);
	QCOLOR (unsigned int *_palette, unsigned char _palindex);
...
public:
	...
	QMATRIX (const float *m);
	void Identity (void);
	void PutZGoingUp (void);
	void Transpose (void);
	void Transpose (D3DXMATRIX *dest);
	void CopyTo (D3DXMATRIX *dest);
	void Translate (float x, float y, float z);
	void Translate (const float *xyz);
	void Rotate (float x, float y, float z, float angle);
	void Rotate (float y, float p, float r);
	void ExtractFrustum (struct mplane_t *f);
	...
	void Mult (const D3DXMATRIX *in);
	void Mult (D3DXMATRIX *out, const D3DXMATRIX *in);
	...
	void Inverse (D3DXMATRIX *other);
	void Inverse (void);
	void AngleVectors (const float *angles);
	static void UpdateMVP (QMATRIX *mvp, const QMATRIX *m, const QMATRIX *p);
};
...
// plane_t structure - pulled out from model because it needs to be common to lotsa things
struct mplane_t
{
	float	normal[3];
	float	dist;
...
public:
	CHostTimer (void);
	void Reset (void);
	void Tick (double standarddelta, double idledelta, bool userdelta);
...
void Host_ClientCommands (struct client_t *client, char *fmt, ...);
...
#define CACHE_ALIGN(siz) (((siz) + Sys.CacheLineMask) & ~Sys.CacheLineMask)

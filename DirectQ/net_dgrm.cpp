...
int NET_BigLong (int l)
{
	return ((l & 255) << 24) + (((l >> 8) & 255) << 16) + (((l >> 16) & 255) << 8) + ((l >> 24) & 255);
}
...
int Datagram_SendMessage (qsocket_t *sock, sizebuf_t *data)
{
	...
	packetBuffer.length = NET_BigLong (packetLen | (NETFLAG_DATA | eom));
	packetBuffer.sequence = NET_BigLong (sock->sendSequence++);
...
int ReSendMessage (qsocket_t *sock)
{
	...
	packetBuffer.length = NET_BigLong (packetLen | (NETFLAG_DATA | eom));
	packetBuffer.sequence = NET_BigLong (sock->sendSequence - 1);
	Q_MemCpy (packetBuffer.data, sock->sendMessage, dataLen);
...
int Datagram_SendUnreliableMessage (qsocket_t *sock, sizebuf_t *data)
{
	...
	packetBuffer.length = NET_BigLong (packetLen | NETFLAG_UNRELIABLE);
	packetBuffer.sequence = NET_BigLong (sock->unreliableSendSequence++);
...
int Datagram_GetMessage (qsocket_t *sock)
{
	...
	for (;;)
	{
		...
		length = NET_BigLong (packetBuffer.length);
		...
		sequence = NET_BigLong (packetBuffer.sequence);
		packetsReceived++;
		...
		{
			packetBuffer.length = NET_BigLong (NET_HEADERSIZE | NETFLAG_ACK);
			packetBuffer.sequence = NET_BigLong (sequence);
...
void Strip_Port (char *ch)
{
	if ((ch = strchr (ch, ':')))
	{
		...
		if (sscanf (ch + 1, "%d", &net_hostport) != 1) return;
...
static void Test_Poll (void *blah)
{
	...
	for (;;)
	{
		...
		MSG_BeginReading ();
		control = NET_BigLong (*((int *) net_message.data));
		MSG_ReadLong ();
...
JustDoIt:
	...
	for (n = 0; n < max; n++)
	{
		...
		*((int *) net_message.data) = NET_BigLong (NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
...
static void Test2_Poll (void *blah)
{
	struct qsockaddr clientaddr;
	int		control, len;
...
static void Rcon_Poll (void *blah)
{
	struct qsockaddr clientaddr;
	int	control, len;
...
JustDoIt:
	...
	*((int *) net_message.data) = NET_BigLong (NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
	net_landrivers[net_landriverlevel].Write (testSocket, net_message.data, net_message.cursize, &sendaddr);
	SZ_Clear (&net_message);
	SchedulePollProcedure (&rconPollProcedure, 0.05f);
}
...
#ifdef BAN_TEST
cmd_t ban_cmd ("ban", NET_Ban_f);
#endif

cmd_t test_cmd ("test", Test_f);
cmd_t test2_cmd ("test2", Test2_f);
cmd_t rcon_cmd ("rcon", Rcon_f);	// joe: rcon from ProQuake
...
static qsocket_t *_Datagram_CheckNewConnections (void)
{
	...
	if (command == CCREQ_RULE_INFO)
	{
		...
		if ((var = cvar_t::GetNextServerRuleVar (prevCvarName)) != NULL)
		{
			// send the response
			SZ_Clear (&net_message);
	...
	if (command == CCREQ_RCON)
	{
		...
		*((int *) rcon_message.data) = NET_BigLong (NETFLAG_CTL | (rcon_message.cursize & NETFLAG_LENGTH_MASK));
	...
	if (mod != MOD_QSMACK)
	{
		if (cl_password.value && (len <= 18 || cl_password.value != MSG_ReadLong ()))
			return Datagram_Reject ("Óåòöåò éó ðáóó÷ïòä ðòïôåãôåäŸ\nYou must use ProQuake v3.1 or a compatible engine\nand set cl_password/pq_password to the server password\n", acceptsock, &clientaddr);
...
static qsocket_t *_Datagram_Connect (char *host)
{
	...
	if (ret == CCREP_ACCEPT)
	{
		...
		else sock->mod = MOD_NONE;

		if (len > 10)
			sock->mod_version = MSG_ReadByte ();
		else sock->mod_version = 0;

		if (len > 11)
			sock->mod_flags = MSG_ReadByte ();
		else sock->mod_flags = 0;

#if 0

		if (sock->mod == MOD_PROQUAKE && (sock->mod_flags & JQF_CHEATFREE))
		{
			if (security_loaded)
			{
				cl_cheatfree = true;

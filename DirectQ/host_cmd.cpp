...
void Host_Status_f (void)
{
	...
	print ("host:    %s\n", cvar_t::VariableString ("hostname"));
	print ("version: %4.2f\n", VERSION);
...
{
	memset (text, ' ', SAVEGAME_COMMENT_LENGTH);
	strcpy (text, cl.levelname);
	strcpy (&text[22], va ("kills:%3i/%3i", cl.stats[STAT_MONSTERS], cl.stats[STAT_TOTALMONSTERS]));
...
void Host_DoSavegame (char *savename)
{
	...
	for (int i = 0; ; i++)
	{
		...
		if (savename[i] == '/' || savename[i] == '\\' || savename[i] == ':' ||
			savename[i] == '*' || savename[i] == '?' || savename[i] == '"' ||
			savename[i] == '<' || savename[i] == '>' || savename[i] == '|')
		{
	...
	Q_snprintf (name, 256, "%s/%s%s", com_gamedir, host_savedir.string, savename);
	...
	savefile << std::fixed;
	savefile << SAVEGAME_VERSION << "\n";
	Host_SavegameComment (comment);
	savefile << comment << "\n";

	for (int i = 0; i < NUM_SPAWN_PARMS; i++)
		savefile << svs.clients->spawn_parms[i] << "\n";

	savefile << current_skill << "\n";
	savefile << sv.name << "\n";
	savefile << sv.time << "\n";
	...
	{
		if (sv.lightstyles[i])
			savefile << sv.lightstyles[i] << "\n";
		else savefile << "m\n";
	}

	ED_WriteGlobals (savefile);

	for (int i = 0; i < SVProgs->NumEdicts; i++)
		ED_Write (savefile, GetEdictForNumber (i));
...
void Host_Loadgame_f (void)
{
	...
	for (int i = 0; ; i++)
	{
		...
		// check for invalid chars
		if (loadname[i] == '/' || loadname[i] == '\\' || loadname[i] == ':' ||
			loadname[i] == '*' || loadname[i] == '?' || loadname[i] == '"' ||
			loadname[i] == '<' || loadname[i] == '>' || loadname[i] == '|')
		{
	...
	Q_snprintf (name, 127, "%s/%s%s", com_gamedir, host_savedir.string, loadname);
	COM_DefaultExtension (name, ".sav");
	...
	if (!f.is_open ())
	{
		...
		Q_snprintf (name, 127, "%s/%s", com_gamedir, loadname);
		COM_DefaultExtension (name, ".sav");
	...
	// safely bring up the loading plaque now
	SCR_BeginLoadingPlaque ();
	...
	// this is nasty because whitespace is skipped before, not after, so we'll read nothing on subsequent reads
	// unless we scan to and consume a newline char
	f >> std::noskipws;
	f >> version;
	...
	{
		f.getline (loadbuffer, LOADGAME_BUFFER_SIZE);
		sv.lightstyles[i] = (char *) MainHunk->Alloc (strlen (loadbuffer) + 1);
		strcpy (sv.lightstyles[i], loadbuffer);
	}

	// load all the edicts out of the savegame file
	// (the memset ensures null-termination of the string)
	memset (loadbuffer, 0, LOADGAME_BUFFER_SIZE);
	f.read (loadbuffer, LOADGAME_BUFFER_SIZE);

	// -1 is the globals
	for (SVProgs->NumEdicts = -1; ; SVProgs->NumEdicts++)
	{
		if ((loadbuffer = COM_Parse (loadbuffer)) == NULL) break;
		if (!com_token[0]) break;
		if (strcmp (com_token, "{")) Sys_Error ("First token isn't a brace");

		if (SVProgs->NumEdicts == -1)
		{
			// parse the global vars
			loadbuffer = ED_ParseGlobals (loadbuffer);
		}
		else
		{
			// parse an edict
			edict_t *ent = GetEdictForNumber (SVProgs->NumEdicts);

			memset (&ent->v, 0, SVProgs->QC->entityfields * 4);
			ent->free = false;
			loadbuffer = ED_ParseEdict (loadbuffer, ent);
...
void Host_Say (bool teamonly)
{
	...
	// turn on color set 1
	if (!fromServer)
		Q_snprintf (text, SAY_BUF_LEN, "%c%s: ", 1, save->name);
	else Q_snprintf (text, SAY_BUF_LEN, "%c<%s> ", 1, hostname.string);
...
void Host_Color_f (void)
{
	...
	h_playercolor = top * 16 + bottom;

	if (cmd_source == src_command)
	{
		cl_color.Set (h_playercolor);
	...
	host_client->colors = h_playercolor;
...
void Host_Kill_f (void)
{
	...
	SVProgs->RunInteraction (sv_player, NULL, SVProgs->GlobalStruct->ClientKill);
...
void Host_Spawn_f (void)
{
	...
	else
	{
		...
		SVProgs->GlobalStruct->time = sv.time;
		SVProgs->RunInteraction (sv_player, NULL, SVProgs->GlobalStruct->ClientConnect);
		SVProgs->RunInteraction (sv_player, NULL, SVProgs->GlobalStruct->PutClientInServer);
	}
	...
	// send the correct angles
	float *sendangle = sv.loadgame ? ent->v.v_angle : ent->v.angles;

	// demos don't go through here...
	if (host_client->netconnection->mod == MOD_PROQUAKE && sv.Protocol == PROTOCOL_VERSION_NQ)
	{
		for (i = 0; i < 2; i++)
			MSG_WriteAngle8_Old (&host_client->message, sendangle[i]);

		MSG_WriteAngle8_Old (&host_client->message, 0);	
	}	
	else
	{
		// no need for angle fixup on the player
		for (i = 0; i < 2; i++)
			MSG_WriteAngle (&host_client->message, sendangle[i], sv.Protocol, sv.PrototcolFlags, 0);

		MSG_WriteAngle (&host_client->message, 0, sv.Protocol, sv.PrototcolFlags, 0);
	}
...
bool unfun_match (char *s1, char *s2)
{
	int i;
...
void Host_Identify_f (void)
{
	...
	if (sv.active)
	{
		...
		Q_strncpy (name, svs.clients[i].name, 15);
		name[15] = 0;
	...
	else
	{
		...
		Q_strncpy (name, cl.scores[i].name, 15);
		name[15] = 0;
		Con_Printf ("known aliases for %s:\n", name);

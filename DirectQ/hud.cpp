...
cvar_t pq_timer_x ("pq_timer_x", "280", CVAR_ARCHIVE);
cvar_t pq_timer_y ("pq_timer_y", "15", CVAR_ARCHIVE);
cvar_t scr_clock_x ("scr_clock_x", "0", CVAR_ARCHIVE);
cvar_t scr_clock_y ("scr_clock_y", "15", CVAR_ARCHIVE);
...
void HUD_DrawNum (int x, int y, int num, int color = 0)
{
	char str[12];
	char *ptr;
	int l, frame;
	...
	while (*ptr)
	{
		...
		Draw_Pic (x, y, sb_nums[color & 1][frame], 1, true);
		x += 24;
		ptr++;
	}
}
...
void HUD_DrawElapsedTime (int x, int y, float time)
{
	...
	Q_snprintf (str, 80, "Time: %i:%i%i", minutes, tens, units);
...
void HUD_DeathmatchOverlay (void)
{
	...
	for (i = 0; i < l; i++)
	{
		...
		if (have_cl_pings)
		{
			Q_snprintf (num, 12, "%4ims", s->ping > 9999 ? 9999 : s->ping);
...
void HUD_SoloScoreboard (qpic_t *pic, float solotime)
{
	...
	if (cl.gametype != GAME_DEATHMATCH)
	{
		...
		if (cl.stats[STAT_TOTALMONSTERS] == 0)
			Q_snprintf (str, 80, "Kills: %i", cl.stats[STAT_MONSTERS]);
		else Q_snprintf (str, 80, "Kills: %i/%i", cl.stats[STAT_MONSTERS], cl.stats[STAT_TOTALMONSTERS]);
		...
		if (cl.stats[STAT_TOTALSECRETS] == 0)
			Q_snprintf (str, 80, "Secrets: %i", cl.stats[STAT_SECRETS]);
		else Q_snprintf (str, 80, "Secrets: %i/%i", cl.stats[STAT_SECRETS], cl.stats[STAT_TOTALSECRETS]);
...
void HUD_DrawFace (int ActiveItems, int HealthStat)
{
	...
	HUD_DrawNum (x, y, HealthStat, HealthStat <= 25);
}
...
void HUD_DrawArmor (int ActiveItems, int ArmorStat)
{
	...
	HUD_DrawNum (x, y, armorval, armorval <= 25 || armorval == 666);
}
...
void HUD_DrawAmmo (int ActiveItems, int AmmoStat)
{
	...
	HUD_DrawNum (x, y, AmmoStat, AmmoStat <= 10);
}
...
void HUD_DrawAmmoCounts (int ac1, int ac2, int ac3, int ac4, int ActiveWeapon)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		Q_snprintf (num, 10, "%3i", ActiveAmmo[i]);
...
void HUD_DrawFPS (void)
{
	if (scr_showfps.value)
	{
		...
		{
			if (d3d_RenderDef.fps < 0)
				Q_snprintf (str, 16, "0 ms");
			else Q_snprintf (str, 16, "%0.3f ms", (1000.0f / d3d_RenderDef.fps));
		}
		else
		{
			if (d3d_RenderDef.fps < 0)
				Q_snprintf (str, 16, "0 fps");
			else Q_snprintf (str, 16, "%i fps", (int) (d3d_RenderDef.fps + 0.5f));
		}

		int len = (strlen (str) * 8) + 4;
		int x = vid.currsize.width - len;

		// Draw_Fill is too expensive here...
		if (cl_sbar.integer > 2)
		{
			// Draw_Fill (x - 2, vid.currsize.height - y1, len, 11, 0, 128);
			Draw_String (x, vid.currsize.height - y1, str);
		}
		else
		{
			// Draw_Fill (x - 2, vid.currsize.height - y2, len, 11, 0, 128);
			Draw_String (x, vid.currsize.height - y2, str);
		}
	}
}
...
void HUD_DrawHUD (void)
{
	...
	// bound sbar
	if (cl_sbar.integer < 0) cl_sbar.Set (0.0f);
	if (cl_sbar.integer > 3) cl_sbar.Set (3.0f);
...
void HUD_DrawStuffGotten (int x, int y, int numstuff, int totalstuff)
{
	...
	for (int i = 0;; i++)
	{
		...
		if (stuffstr[i] == '/')
		{
			...
			x += 24;
		else if (stuffstr[i] < '0')
			continue;
		else if (stuffstr[i] > '9')
			continue;
		else
		{

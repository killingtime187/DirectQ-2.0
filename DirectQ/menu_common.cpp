...
CQMenuSpinControl::CQMenuSpinControl (char *commandtext, cvar_t *menucvar, float minval, float maxval, float increment, char *zerotext, char *units)
{
	...
	if (zerotext)
	{
		this->ZeroText = (char *) MainZone->Alloc (strlen (zerotext));
...
void CQMenuSpinControl::DrawCurrentOptionHighlight (int y)
{
	...
	if (drawleft) Menu_DrawBackwardsCharacter (lpos, y, 12 + ((int) (CHostTimer::realtime * 2) & 1));
	if (drawright) Menu_DrawCharacter (rpos, y, 12 + ((int) (CHostTimer::realtime * 2) & 1));
}
...
void CQMenuSpinControl::Draw (int y)
{
	...
	if (this->MenuCvar)
	{
		...
		{
			Q_snprintf (this->OutputText, 256, "%g", this->MenuCvar->value);
...
void CQMenuSpinControl::Key (int k)
{
	...
	if (this->MenuCvar)
	{
		...
		// update the cvar
		this->MenuCvar->Set (this->MenuCvar->value);
	}
	else if (this->StringBuf)
	{
		switch (k)
...
CQMenuCvarTextbox::CQMenuCvarTextbox (char *commandtext, cvar_t *menucvar, int flags)
{
	...
	this->InitialValue = (char *) MainZone->Alloc (2);
	this->ScratchPad = (char *) MainZone->Alloc (1024);
	this->WorkingText = (char *) MainZone->Alloc (1024);
...
void CQMenuCvarTextbox::Draw (int y)
{
	...
	if (strcmp (this->MenuCvar->string, this->WorkingText))
		this->MenuCvar->Set (this->WorkingText);
	...
	{
		// edit cursor
		Menu_DrawCharacter (170 + 8 * this->TextPos, y, 10 + ((int) (CHostTimer::realtime * 4) & 1));
...
void CQMenuCvarTextbox::PerformEntryFunction (void)
{
	...
	MainZone->Free (this->InitialValue);
	this->InitialValue = (char *) MainZone->Alloc (strlen (this->MenuCvar->string) + 1);
...
{
	Draw_Pic (((vid.currsize.width - 240) >> 1) + 5, y, menu_dot_lmp[(int) (CHostTimer::realtime * 10) % 6], 1, true);
}
...
void CQMenuCvarToggle::Key (int k)
{
	...
	case K_ENTER:
		...
		// set the cvar
		this->MenuCvar->Set (this->ToggleState ? this->ToggleOnValue : this->ToggleOffValue);
		break;
...
void CQMenuOption::AllocCommandText (char *commandtext)
{
	if (commandtext)
	{
		this->MenuCommandText = (char *) MainZone->Alloc (strlen (commandtext) + 1);
		strcpy (this->MenuCommandText, commandtext);
	}
	else
	{
		this->MenuCommandText = (char *) MainZone->Alloc (2);
...
void Menu_DrawOption (int x, int y, char *option, bool leftflash, bool rightflash)
{
	...
	if (leftflash) Menu_DrawBackwardsCharacter (x - 12, y, 12 + ((int) (CHostTimer::realtime * 2) & 1));
	if (rightflash) Menu_DrawCharacter (x + 4 + strlen (option) * 8, y, 12 + ((int) (CHostTimer::realtime * 2) & 1));
}
...
void Menu_HighlightGeneric (int x, int y, int width, int height)
{
	...
	int color = colorramp[(int) (CHostTimer::realtime * 10) % 8];
...
void M_Draw (void)
{
	...
	if (menu_fillcolor.integer < 0) menu_fillcolor.Set (0.0f);
	if (menu_fillcolor.integer > 13) menu_fillcolor.Set (13.0f);
	...
	D3DDraw_SetRect (0, 0, vid.menusize.width, vid.menusize.height);
	...
	D3DDraw_SetRect (0, (vid.menusize.height - 480) / 2, vid.menusize.width, vid.menusize.height);

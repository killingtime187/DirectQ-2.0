...
void FreeSound (void)
{
	...
	if (ds_Device)
	{
		ds_Device->SetCooperativeLevel (vid.Window, DSSCL_NORMAL);
		ds_Device->Release ();
	}
...
BOOL CALLBACK DSEnumCallback (LPGUID lpGuid, LPCSTR lpcstrDescription, LPCSTR lpcstrModule, LPVOID lpContext)
{
	...
	{
		Q_strncpy (ds_Devices[ds_NumDevices].Description, lpcstrDescription, 127);
		Q_strncpy (ds_Devices[ds_NumDevices].Module, lpcstrModule, 63);
		Q_MemCpy (&ds_Devices[ds_NumDevices].DevGuid, lpGuid, sizeof (GUID));
...
sndinitstat SNDDMA_InitDirect (void)
{
	...
	ds_BestModule[0] = 0;
	Q_MemCpy (&ds_BestGuid, &DSDEVID_DefaultPlayback, sizeof (GUID));
	ds_BestBuffers = 0;
	ds_BestSampleRate = 0;
	...
	if (ds_device.integer < 0) ds_device.Set (0.0f);
	if (ds_device.integer >= MAX_DS_DEVICES) ds_device.Set ((float) (MAX_DS_DEVICES - 1));

#pragma warning(suppress: 6385)
	if (!ds_Devices[ds_device.integer].Valid)
	{
		// copy best valid device out
		Q_strncpy (ds_Devices[ds_device.integer].Description, ds_BestDescription, 127);
		Q_strncpy (ds_Devices[ds_device.integer].Module, ds_BestModule, 63);
		Q_MemCpy (&ds_Devices[ds_device.integer].DevGuid, &ds_BestGuid, sizeof (GUID));
...
#pragma warning(suppress: 6386)
	...
	if (DS_OK != ds_Device->SetCooperativeLevel (vid.Window, DSSCL_PRIORITY))
	{

...
DWORD Web_DoDownload (char *url, char *file, DOWNLOADPROGRESSPROC progress)
{
	...
	if (!(hURL = QInternetOpenUrl (hInternet,
		url,
		NULL,
		0,
	...
	fDownload.open (TempPath, std::ios::out | std::ios::binary);

	if (!fDownload.is_open ())
	{
	...
	for (;;)
	{
		...
		// write out
		fDownload.write ((char *) ReadBuffer, BytesRead);
	}

	// success also falls through here
cleanup_fail:;
	if (fDownload.is_open ()) fDownload.close ();

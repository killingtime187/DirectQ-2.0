...
int Menu_KeybindingsCustomDraw (int y)
{
	...
	for (int i = 0; i < ARRAYLENGTH (keybinds); i++)
	{
...
void Menu_KeybindingsCustomKey (int key)
{
	...
	case K_UPARROW:
		...
		if ((--bind_cursor) < 0) bind_cursor = ARRAYLENGTH (keybinds) - 1;
	...
	case K_DOWNARROW:
		...
		if ((++bind_cursor) >= ARRAYLENGTH (keybinds)) bind_cursor = 0;
...
// these are just dummy cvars for operating the menu options now
cvar_t com_hipnotic;
cvar_t com_rogue;
cvar_t com_quoth;
cvar_t com_nehahra;
...
void Menu_GameCustomEnter (void)
{
	...
	if (hipnotic) com_hipnotic.Set (1.0f); else com_hipnotic.Set (0.0f);
	if (rogue) com_rogue.Set (1.0f); else com_rogue.Set (0.0f);
	if (quoth) com_quoth.Set (1.0f); else com_quoth.Set (0.0f);
	if (nehahra) com_nehahra.Set (1.0f); else com_nehahra.Set (0.0f);
}
...
void Menu_GameLoadGames (void)
{
	...
	com_loadquoth = com_quoth.integer ? true : false;
	com_loadrogue = com_rogue.integer ? true : false;
	com_loadhipnotic = com_hipnotic.integer ? true : false;
	com_loadnehahra = com_nehahra.integer ? true : false;
...
void EnumGameDirs (void)
{
	...
	do
	{
		...
		// store it out
		gamedirs[numgamedirs] = (char *) MainZone->Alloc (strlen (FindFileData.cFileName) + 1);
...
int Menu_EffectsCustomDraw (int y)
{
	...
	if (r_lerpframe.integer < 0) r_lerpframe.Set ((float) 0);
	if (r_lerpframe.integer > 1) r_lerpframe.Set (1);
...
#define TAG_FULLBRIGHT		32768
#define TAG_NEHDISABLE		65536
...
char *waterwarp_options[] = {"Off", "Modern", "Perspective", "Classic", "FitzQuake", NULL};
int waterwarp_num = 1;
...
{
	r_waterwarp.Set (waterwarp_num);
	r_particlestyle.Set (particle_num);

	if (!nehahra)
	{
		if (overbright_num == 0)
		{
			r_overbright.Set (0.0f);
			r_hdrlight.Set (0.0f);
		}
		else if (overbright_num == 1)
		{
			r_overbright.Set (1.0f);
			r_hdrlight.Set (0.0f);
		}
	...
	if (flashblend_num == 0)
	{
		// lightmaps only
		r_coronas.Set (0.0f);
		gl_flashblend.Set (0.0f);
	}
	else if (flashblend_num == 1)
	{
		// coronas only
		r_coronas.Set (0.0f);
		gl_flashblend.Set (1.0f);
	}
	else
	{
		// lightmaps + coronas
	...
	if (!hudstyle) return y;

	hudstyle->Set (hudstyleselection);
	...
	if (nehahra)
	{
		menu_EffectsSimple.DisableMenuOptions (TAG_NEHDISABLE);
		menu_EffectsSimple.DisableMenuOptions (TAG_FULLBRIGHT);
	}
	else
	{
		menu_EffectsSimple.EnableMenuOptions (TAG_NEHDISABLE);

		if (gl_fullbrights.integer)
			menu_EffectsSimple.EnableMenuOptions (TAG_FULLBRIGHT);
		else menu_EffectsSimple.DisableMenuOptions (TAG_FULLBRIGHT);
	}

	if (hud_invshownum)
		scr_viewsize.Set (110.0f);
	else scr_viewsize.Set (100.0f);
...
{
	menu_fillcolor.Set (menu_fillcolor.integer & 15);
	r_menucolor.Set (r_menucolor.integer & 15);
	...
	GETCVAROPTION (r_waterwarp.integer, waterwarp_num, 0, 4);
	...
	if (nehahra)
		overbright_num = 0;
	else if (r_overbright.integer && r_hdrlight.integer)
		overbright_num = 2;
...
int Menu_OptionsCustomDraw (int y)
{
	...
	menu_fillcolor.Set (menu_fillcolor.integer);
	r_menucolor.Set (r_menucolor.integer);
...
int Menu_SpeedDrawCheck (int y)
{
	...
	if (dummy_speed_backup != dummy_speed.integer)
	{
		if (dummy_speed.integer)
		{
			...
			if (cl_forwardspeed.value < 400) cl_forwardspeed.Set (400);
			if (cl_backspeed.value < 400) cl_backspeed.Set (400);
		}
		else
		{
			// only reset these if they are higher than the base speed, otherwise keep the lower value
			if (cl_forwardspeed.value > 200) cl_forwardspeed.Set (200);
			if (cl_backspeed.value > 200) cl_backspeed.Set (200);
		}
...
void Menu_InitOptionsMenu (void)
{
	...
	menu_Options.AddOption (new CQMenuSpacer (DIVIDER_LINE));
	menu_Options.AddOption (new CQMenuCvarSlider ("Brightness", &v_gamma, 1.75f, 0.25f, 0.05f));
	menu_Options.AddOption (new CQMenuCvarSlider ("Contrast", &v_contrast, 0.25f, 1.75f, 0.05f));
	menu_Options.AddOption (new CQMenuCvarSlider ("Music Volume", &bgmvolume, 0.0f, 1.0f, 0.05f));
	menu_Options.AddOption (new CQMenuCvarSlider ("Sound Volume", &volume, 0.0f, 1.0f, 0.05f));
	menu_Options.AddOption (new CQMenuSpacer (DIVIDER_LINE));
	menu_Options.AddOption (new CQMenuCvarSlider ("Mouse Speed", &sensitivity, 1.0f, 21.0f, 1.0f));
	menu_Options.AddOption (new CQMenuCvarToggle ("Mouse Look", &freelook, 0, 1));
	menu_Options.AddOption (new CQMenuCvarToggle ("Invert Mouse", &m_pitch, 0.022f, -0.022f));
	...
	menu_UI.AddOption (new CQMenuColourBar ("Menu Background", &r_menucolor.integer));
	menu_UI.AddOption (TAG_CONSCALE, new CQMenuCvarSlider ("Console Size", &gl_conscale, 1, 0, 0.1f));
	...
	menu_UI.AddOption (TAG_HUDALPHA, new CQMenuCvarSlider ("HUD Alpha", &scr_sbaralpha, 0, 1, 0.1f));
	...
	menu_Sound.AddOption (new CQMenuCvarSlider ("Music Volume", &bgmvolume, 0, 1, 0.05f));
	menu_Sound.AddOption (new CQMenuCvarSlider ("Sound Volume", &volume, 0, 1, 0.05f));
	...
	menu_Sound.AddOption (new CQMenuCvarSlider ("Ambient Level", &ambient_level, 0, 1, 0.05f));
	...
	menu_EffectsSimple.AddOption (TAG_NEHDISABLE, new CQMenuSpinControl ("Overbright Lighting", &overbright_num, overbright_options));
	...
	menu_EffectsSimple.AddOption (new CQMenuTitle ("Fullbright Colors"));
	menu_EffectsSimple.AddOption (TAG_NEHDISABLE, new CQMenuCvarToggle ("Show Fullbrights", &gl_fullbrights, 0, 1));
	menu_EffectsSimple.AddOption (TAG_FULLBRIGHT, new CQMenuCvarToggle ("Additive Mode", &r_additivefullbrights, 0, 1));
	...
	menu_EffectsSimple.AddOption (new CQMenuCvarSlider ("Particle Size", &r_particlesize, 0.25f, 5.0f, 0.25f));

	menu_EffectsSimple.AddOption (new CQMenuTitle ("Water and Liquids"));
	menu_EffectsSimple.AddOption (new CQMenuCvarSlider ("Water Alpha", &r_wateralpha, 0.0f, 1.0f, 0.1f));
	menu_EffectsSimple.AddOption (new CQMenuSpinControl ("Underwater Warp", &waterwarp_num, waterwarp_options));
	menu_EffectsSimple.AddOption (new CQMenuCvarSlider ("Underwater Fog", &gl_underwaterfog, 0.0f, 5.0f, 0.25f));
...
void Menu_HelpCustomEnter (void)
{
	if (nehahra && !menu_advanced.integer)
	{
		...
		if (Sys_FileExists (va ("%s/endcred.dem", com_gamedir)))
		{
			nehdemo = true;
			key_dest = key_game;
...
void Menu_MapsCacheInfo (mapinfo_t *info, char *entlump)
{
	...
	for (;;)
	{
		...
		{
			info->mapname = (char *) GameHunk->Alloc (strlen (com_token) + 1);
		...
		{
			info->skybox = (char *) GameHunk->Alloc (strlen (com_token) + 1);
			strcpy (info->skybox, com_token);
		}
		else if (!_stricmp (key, "wad"))
		{
			info->wad = (char *) GameHunk->Alloc (strlen (com_token) + 1);
...
void Menu_MapsOnHover (int initialy, int y, int itemnum)
{
	...
	Draw_Mapshot ("maps/", menu_mapslist[itemnum].bspname, (vid.currsize.width - 320) / 2 + 208, initialy + 55);
...
bool ValidateMap (char *mapname, int itemnum)
{
	...
	Q_snprintf (fullmapname, 260, "maps/%s", mapname);

	if (!mapfile.Open (fullmapname)) return false;

	// because the map could be coming from a PAK file we need to save out the base pos so that fseek is valid
	int basepos = mapfile.SetPointer (0, FILE_CURRENT);
	dheader_t bsphead;

	if (mapfile.Read (&bsphead, sizeof (dheader_t)) != sizeof (dheader_t))
	{
		mapfile.Close ();
		return false;
	}
	...
	mapfile.SetPointer (basepos + bsphead.lumps[LUMP_ENTITIES].fileofs, FILE_BEGIN);

	// read it all in
	char *entlump = (char *) MainZone->Alloc (bsphead.lumps[LUMP_ENTITIES].filelen);

	if (mapfile.Read (entlump, bsphead.lumps[LUMP_ENTITIES].filelen) != bsphead.lumps[LUMP_ENTITIES].filelen)
	{
		mapfile.Close ();
		MainZone->Free (entlump);
		return false;
	}
	else mapfile.Close ();
	...
	for (int i = 0; i < (bsphead.lumps[LUMP_ENTITIES].filelen - 11); i++)
	{
		...
		{
			// map is valid
			menu_mapslist[itemnum].bspname = (char *) GameHunk->Alloc (strlen (mapname) + 1);
...
void Menu_MapsPopulate (void)
{
	...
	int maplistlen = 0;
	menu_mapslist = (mapinfo_t *) GameHunk->Alloc (listlen * sizeof (mapinfo_t));
	...
	spinbox_maps = (char **) GameHunk->Alloc ((num_menumaps + 1) * sizeof (char *));
	spinbox_bsps = (char **) GameHunk->Alloc ((num_menumaps + 1) * sizeof (char *));
...
void Menu_DemoPopulate (void)
{
	...
	demolist = (char **) GameHunk->Alloc ((numdemos + 1) * sizeof (char *));
	...
	if (numdemos)
	{
		for (int i = 0; i < numdemos; i++)
		{
			...
			demolist[i] = (char *) GameHunk->Alloc (thisdemolen + 1);
...
bool M_Menu_Demo_Info (char *demoname)
{
	CQuakeFile demofile;
	int msgsize;
	float viewangs[3];
	...
	if (!demofile.Open (va ("%s.dem", demoname)))
	{
		// try opening in /demos
		if (!demofile.Open (va ("demos/%s.dem", demoname)))
		{
			// didn't get it
			return false;
		}
	...
	for (;;)
	{
		byte *msgdata = (byte *) scratchbuf;

		// get the size of the message
		if (demofile.Read (&msgsize, 4) != 4) break;
		if (demofile.Read (viewangs, sizeof (float) * 3) != sizeof (float) * 3) break;
		if (demofile.Read (msgdata, msgsize) != msgsize) break;
		...
		for (int msgpos = 0;;)
		{
			...
			case svc_serverinfo:
				...
				// the cd track will be stomped on by this read, so we need to save it out then restore it
				{
...
int Menu_DemoCustomDraw2 (int y)
{
	...
	if (democmd_num == 2)
	{
		Draw_Mapshot ("maps/", spinbox_bsps[demo_mapnum], (vid.currsize.width - 320) / 2 + 96, y);
		y += 130;
	...
	else
	{
		...
		if (dsi.mapname)
			Draw_Mapshot ("maps/", dsi.mapname, (vid.currsize.width - 320) / 2 + 96, y);
		else Draw_Mapshot (NULL, NULL, (vid.currsize.width - 320) / 2 + 96, y);
...
{
	y += 5;
	Draw_Mapshot ("maps/", spinbox_bsps[menumapselectedbsp], (vid.currsize.width - 320) / 2 + 96, y);
	y += 130;

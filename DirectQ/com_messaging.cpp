...
void SZ_Alloc (sizebuf_t *buf, int startsize)
{
	...
	buf->data = (byte *) MainZone->Alloc (startsize);
...
{
	Q_MemCpy (SZ_GetSpace (buf, length, "SZ_Write"), data, length);
}
...
void SZ_Print (sizebuf_t *buf, char *data)
{
	...
	if (buf->data[buf->cursize-1])
		Q_MemCpy ((byte *) SZ_GetSpace (buf, len, "SZ_Print : buf->data[buf->cursize-1]"), data, len); // no trailing 0
	else Q_MemCpy ((byte *) SZ_GetSpace (buf, len - 1, "SZ_Print : other") - 1, data, len); // write over trailing 0
}

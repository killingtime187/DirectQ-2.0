...
BOOL PASCAL FAR BlockingHook (void)
{
	...
	if ((Sys_DoubleTime () - dBlockTime) > 2)
	{
...
void WINS_GetLocalAddress (void)
{
	...
	addr = ntohl (myAddr);
	Q_snprintf (my_tcpip_address, 64, "%d.%d.%d.%d", (addr >> 24) & 0xff, (addr >> 16) & 0xff, (addr >> 8) & 0xff, addr & 0xff);
}
...
char *WINS_AddrToString (struct qsockaddr *addr)
{
	...
	Q_snprintf (buffer, 22, "%d.%d.%d.%d:%d", (haddr >> 24) & 0xff, (haddr >> 16) & 0xff, (haddr >> 8) & 0xff, haddr & 0xff, ntohs (((struct sockaddr_in *) addr)->sin_port));
	return buffer;
}

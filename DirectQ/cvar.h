...
// normally not archived but should be added to the config via a seta command
#define CVAR_SETA			8192
...
public:
	...
	static cvar_t *FindVar (char *var_name);
	static char *VariableString (char *var_name);
	static float VariableValue (char *var_name);

	// allow adding cvars explicitly within functions too
	static void AddVariable (char *cvarname, char *initialval, int useflags = 0, cvarcallback_t cb = NULL);
	static void AddVariable (char *cvarname, float initialval, int useflags = 0, cvarcallback_t cb = NULL);
	static void AddVariable (void);

	static void ResetAll (void);
	static void WriteVariables (std::ofstream &f);
	static cvar_t *GetNextServerRuleVar (char *prevCvarName);
...
#define Cvar_Get(var, varname) static cvar_t *(var) = NULL; if (!(var)) (var) = cvar_t::FindVar (varname);if (!(var)) Sys_Error ("%s is not a cvar", varname);
